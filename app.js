...
// Currently pointed at the simple backend's POST /api/query ({text} in, {provider,answer,sources,rag_used} out).
// If you switch to the fuller orchestrator backend (POST /api/v1/chat, {message,profile} in,
// {answer,intent,stage,confidence,actions,sources,warnings,metadata} out), change this to '/api/v1/chat'
// and change the payload built in ask() below to {message:q, profile:buildRelocationProfile()}.
...
function buildRelocationProfile(){
const persona=PERSONAS[currentPersona]||{};
const sub=persona.sub||'';
const arrowMatch=sub.match(/\u2192\s*([A-Za-z\u00C0-\u00FF\s]+)$/);
const destination_city=arrowMatch?arrowMatch[1].trim():null;
const employment_status=currentPersona==='Professional'?'employed':(currentPersona==='Researcher'?'researcher':(currentPersona==='Student'?'student':null));
const spouse=!!(typeof famAnswers!=='undefined'&&famAnswers&&(famAnswers.family==='spouse'||famAnswers.family==='kids'));
const children=(typeof famAnswers!=='undefined'&&famAnswers&&famAnswers.ages)?famAnswers.ages.length:0;
return{origin_country:'India',destination_city:destination_city,spouse:spouse,children:children,employment_status:employment_status}
}
// Folds profile context into the question text, since the simple /api/query
// endpoint only accepts a single "text" field (no separate profile object).
function buildContextualMessage(q){
const p=buildRelocationProfile();
const bits=[];
if(p.destination_city)bits.push('destination: '+p.destination_city);
if(p.employment_status)bits.push('status: '+p.employment_status);
if(p.spouse)bits.push('traveling with spouse');
if(p.children)bits.push(p.children+' child'+(p.children>1?'ren':''));
if(!bits.length)return q;
return '[User context — '+bits.join(', ')+'] '+q;
}
...
const payload={text:buildContextualMessage(q)};
try{
...
thinking.innerHTML=chatEscapeHtml(data.answer||'')+renderChatMeta(data);
}catch(err){
